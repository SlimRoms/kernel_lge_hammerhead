...
static int dwc3_otg_set_power(struct usb_phy *phy, unsigned mA)
{
...
#ifdef CONFIG_FORCE_FAST_CHARGE
	/* Yank555.lu : Remember we stopped charging */
	...
	/* NB : DWC3_IDEV_CHG_MAX = 1500 / DWC3_IDEV_CHG_MIN = 500 */
	if (force_fast_charge == FAST_CHARGE_FORCE_AC) {/* We are in basic Fast Charge mode, so we substitute AC to USB levels */
		switch(dotg->charger->chg_type) {
			...
			case DWC3_CDP_CHARGER:		// charging downstream port
							...
							current_charge_level = mA; /* Yank555.lu : Remember we are fast charging */
							break;
	...
	// Yank555.lu : Don't fastcharge when slimport device is connected, it's safer
